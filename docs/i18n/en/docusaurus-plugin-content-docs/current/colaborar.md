...
**1 human author identities** sign commits in this branch: `Ubiracy`. Automated identities are excluded. A Git author name is not necessarily one unique person.
...
**9 registered maps** - 2 open in rounds and 7 in capture. `ctfMode` sets the initial mode; it does not lock it. There are 11 `map_*.js` files on disk, so a file alone does **not** make a map playable.
